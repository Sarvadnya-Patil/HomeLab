...
1. **Direct DRM/KMS Scanout (`libdrmtap`)**:
   ...
   - When this tier fails, the daemon logs why as `[SafeDisplayGrabber] libdrmtap capture unavailable: <reason>` (library not found, `drmtap_open` returned NULL, or a grab error code), once per distinct reason and then at most once a minute, and shows the same reason in the dashboard's capture diagnostics. If the capture context could not be opened, for example because the daemon started before the display was ready, it is retried every 10 seconds instead of staying on the slower tiers until the service restarts. `libdrmtap capture recovered` is logged when it starts working again.
...
**Codec negotiation.** The hardware encoder handles H.264 only. `aiortc` takes the codec from the order the browser lists them in its offer, so a stream can end up on VP8, which is always encoded in software. The daemon logs the codec it actually got once per stream, for example `Negotiated codec video/H264; encoding in hardware via /dev/dri/renderD128.` or `Negotiated codec video/VP8; encoding in software because the hardware encoder handles H.264 only.`, and the dashboard's diagnostics show the same codec name. If you expect hardware encoding and see the VP8 line, the browser preferred VP8. During a stream that is in hardware, an `ffmpeg` process is running (`pgrep -a ffmpeg`).

**One offer per connection.** The daemon builds a brand-new peer connection for every offer it receives and closes the previous one. The browser must therefore send exactly one offer per connection attempt, and re-send that same offer only if it went out before the daemon was listening and is still unanswered. Creating a second offer replaces a working connection with one the browser never applied (the browser rejects its answer with `Called in wrong state: stable`), which looks like a stream that starts and then dies. The dashboard tells the browser `daemon_online` the moment it connects, which is why the client ignores that message unless an unanswered offer is waiting.
