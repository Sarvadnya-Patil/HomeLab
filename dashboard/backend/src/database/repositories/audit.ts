...
export class AuditRepository {
  ...
  findAll(limit: number = 100): AuditEntry[] {
    return this.db.all<any>('SELECT * FROM audit_log ORDER BY id DESC LIMIT ?', limit).map(this._mapRow);
  }

  log(userId: string, action: string, entityType: string, entityId: string | null, details: Record<string, any> = {}, ipAddress: string = ''): AuditEntry {
